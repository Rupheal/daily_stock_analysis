"""Sanitized attribution of Run058H semantic blockers from an already restored private tree.

No provider/network calls. Never prints or writes raw prompt/response/model text.
"""
...
def main():
    ...
    pre=json.loads(p.read_text()); inp=json.loads(i.read_text()); analyzer=json.loads(ar.read_text()); final=json.loads(fr.read_text()); post=json.loads(pc.read_text())
    ...
    disposition=('PERMANENT_ISOLATE_NO_RETRY' if (provider_sem002 or analyzer_sem002)
                 else 'DOWNSTREAM_SEM002_REQUIRES_SEPARATE_CONTRACT' if final_sem002
                 else 'NO_SEM002_IN_CHECKED_LAYERS')
    out={
      'schema_version':1,'code':'00001','private_restore_hash_verified':True,
      'provider_response_sha256':hashlib.sha256(pr.read_bytes()).hexdigest(),
      'provider_sem002_pattern_present':provider_sem002,
      ...
      'existing_release_gate_sem001_only':True,'disposition':disposition,
      'repeat_model_request_permitted':False,'model_http_requests_added':0,'raw_content_public':False
    }
    a.out.write_text(json.dumps(out,ensure_ascii=False,indent=2,sort_keys=True)+'\n')
    print(json.dumps({'provider_sem002':provider_sem002,'analyzer_sem002':analyzer_sem002,'pipeline_sem002':final_sem002,'disposition':disposition,'model_requests_added':0}))
