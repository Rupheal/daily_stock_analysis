"""Run004 U45 bounded DeepSeek ranking pilot.

Public-safe by construction: no prompt, raw provider response, free-form model text,
or reasoning content is persisted or printed. Every U member remains in the
...
SCHEMA_VERSION = "run004-u45-safe-v1"
...
def load_inputs(db_path, universe_path, integrity_path, target):
    ...
    if integrity.get("track") != "U" or integrity.get("denominator") != len(members):
...
def validate_model_result(value):
    if not isinstance(value, dict) or set(value) != {"score", "stance", "confidence", "reason_codes"}:
        raise ValueError("invalid_model_schema")
    score = value["score"]
    if isinstance(score, bool) or not isinstance(score, int) or not 0 <= score <= 100:
        raise ValueError("invalid_score")
    if value["stance"] not in STANCES or value["confidence"] not in CONFIDENCES:
    ...
    return {"score": score, "stance": value["stance"], "confidence": value["confidence"], "reason_codes": reasons}
...
def execute(args, post_json=_post_json):
    ...
    universe, integrity, members, coverage, db_hash = load_inputs(args.db, args.universe, args.integrity, args.target)
    integrity_hash = file_sha256(args.integrity)
    universe_hash = file_sha256(args.universe)
    config = {
        "schema": SCHEMA_VERSION, "model": args.model, "max_tokens": MAX_OUTPUT_TOKENS,
    ...
    finished = utcnow()
    package = {
        "schema_version": 1, "run_id": args.run_id, "status": "SAFE_U45_PILOT_COMPLETE",
        "generated_at": finished, "as_of": args.target, "model": args.model,
        ...
        "usage": usage_total,
        "cost": {"basis": "peak price guard; estimate, not invoice", "estimated_peak_cny": round(estimated_peak_cost, 8), "hard_cap_cny": args.max_cny},
        "privacy": {"raw_prompt_saved": False, "raw_provider_response_saved": False,
                    "reasoning_saved": False, "free_form_model_text_saved": False},
        "started_at": started,
    }
    forbidden = ("raw_response", "prompt", "reasoning", "messages", "provider_response")
    ...
    (out/"u45-safe-ranking.json").write_text(json.dumps(package, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    manifest = {
        "run_id": args.run_id,
        "files": {"u45-safe-ranking.json": file_sha256(out/"u45-safe-ranking.json"),
    ...
    print("RUN004_U45_SAFE", json.dumps({"run_id": args.run_id, "denominator": len(members),
...
def parser():
    ...
    p.add_argument("--run-id", default="TRI-DSA-DEV-20260913-004-U45")
    p.add_argument("--max-cny", type=float, default=5.0)
    p.add_argument("--max-requests", type=int, default=36)
