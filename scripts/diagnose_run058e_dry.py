...
def main():
    ...
    result={"schema_version":1,"run_id":"TRI-DSA-O-DRY-DIAG-20260918-058F","returncode":p.returncode,
            "stdout_tail":p.stdout.splitlines()[-80:],"stderr_tail":p.stderr.splitlines()[-80:],
            "model_http_requests":0,"provider_credentials_used":False,"DeepSeek_API_cost_cny":0,
            "drive_claims":0,"real_orders":0}
    bp=a.out/"dry"/"budget.json"
    if bp.exists(): result["budget"]=json.loads(bp.read_text())
    sp=a.out/"dry"/"probe-summary.json"
    if sp.exists(): result["probe_summary"]=json.loads(sp.read_text())
    (a.out/"RUN058F_RESULT.json").write_text(json.dumps(result,ensure_ascii=False,indent=2)+"\n")
