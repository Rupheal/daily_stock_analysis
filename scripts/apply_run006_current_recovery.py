...
def main():
    ...
    def one(m):
        ...
        try:
            ...
            f=yf.Ticker(f'{int(code):04d}.HK').history(start=start.date().isoformat(),end=end.date().isoformat(),auto_adjust=False,actions=True,repair=False,timeout=20)
            yr=[]
            ...
            yp=rawdir/(code+'-yahoo-raw.json');yp.write_text(json.dumps({'provider':'Yahoo yfinance auto_adjust=False repair=False','retrieved_at':datetime.now(timezone.utc).isoformat(),'source_locator':f'https://query1.finance.yahoo.com/v8/finance/chart/{int(code):04d}.HK','rows':yr},allow_nan=False));r['yahoo_raw_sha256']=digest(yp)
            response=requests.get('https://web.ifzq.gtimg.cn/appstock/app/hkfqkline/get',params={'param':f'hk{code},day,,,180'},timeout=20);response.raise_for_status();raw=response.content;tp=rawdir/(code+'-tencent-raw.json');tp.write_bytes(raw);r['tencent_raw_sha256']=digest(tp);r['tencent_raw_source']=response.url;r['raw_available_at']=datetime.now(timezone.utc).isoformat();bb=response.json()['data']['hk'+code].get('day') or [];tr=[dict(zip(('date','open','close','high','low','volume'),b[:6])) for b in bb if b[0]<=target];tr=recent(tr,target)
