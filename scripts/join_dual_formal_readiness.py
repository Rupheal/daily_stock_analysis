...
def assemble(o,u,coverage,data,news,target,provider=None):
 n=o.get('member_count')
 if o:assert n and n==len(o['members']) and o.get('full_union_verified') is True
 ...
 provider=provider or {};pr={x['code']:x for x in provider.get('members',[])}
 ...
 for m in u['members']:
  ...
  independent=provider.get('target_session')==target and pr.get(code,{}).get('provider_target_crosscheck') is True
  if not independent:reasons.append('INDEPENDENT_PRICE_VOLUME_RECONCILIATION_REQUIRED')
  reasons+=['CORPORATE_ACTION_SUSPENSION_AND_TRADE_PLAN_REQUIRED','U_MODEL_OUTPUT_NOT_YET_ACCEPTED']
  urs.append({'code':code,'identity_ready':m.get('identity_verified') is True,'current_buy_eligible':bool(m.get('channels')),'price_data_ready':same and dr.get('price_ready') is True,'news_retrieval_ready':nr.get('retrieval_ready') is True,'news_relevance_ready':nr.get('relevance_ready') is True,'formal_news_ready':nr.get('formal_news_ready') is True,'independent_price_volume_ready':independent,'formal_accepted':False,'missing_reasons':reasons})
 return {'run_id':'TRI-DSA-EXEC-20260917-032','price_session':target,'membership_session':o.get('effective_session'),'O_universe_error':None if o else 'CURRENT_OFFICIAL_UNION_NOT_AVAILABLE','O':{'denominator':n,'native_data_ready':sum(x['native_data_ready'] for x in rows),'native_accepted':0,'members':rows},'U':{'denominator':45,'current_buy_eligible':sum(x['current_buy_eligible'] for x in urs),'independent_price_volume_ready':sum(x['independent_price_volume_ready'] for x in urs),'price_data_ready':sum(x['price_data_ready'] for x in urs),'news_retrieval_ready':sum(x['news_retrieval_ready'] for x in urs),'formal_accepted':0,'members':urs},'model_http_requests':0,'paid_data_calls':0,'qualified_signals':0,'authority_journal_writes':0,'historical_predictions_modified':False}
def main():
 p=argparse.ArgumentParser()
 for k in ('o-universe','u-universe','o-coverage','u-data','u-news','target','out'):p.add_argument('--'+k,required=True)
 p.add_argument('--u-provider');a=p.parse_args();provider,provider_sha=read(a.u_provider) if a.u_provider else ({},None);names=['o_universe','u_universe','o_coverage','u_data','u_news'];blobs=[read(getattr(a,k)) for k in names];r=assemble(*(x[0] for x in blobs),a.target,provider);r['provider_evidence_sha256']=provider_sha;r['input_sha256']={k:x[1] for k,x in zip(names,blobs)};Path(a.out).write_text(json.dumps(r,ensure_ascii=False,indent=2)+'\n');print(json.dumps({k:({x:y for x,y in v.items() if x!='members'} if isinstance(v,dict) else v) for k,v in r.items() if k!='input_sha256'}))
