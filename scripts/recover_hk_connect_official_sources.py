...
def validate(name, data, session):
    ...
    if name == "sse-list.json":
        ...
        if not rows or {x.get("UPDATE_DATE") for x in rows} != {session}:
            raise ValueError("SSE_SESSION_UNVERIFIED")
        if len(rows) != int(d["pageHelp"]["total"]) or int(d["pageHelp"]["pageCount"]) != 1:
            raise ValueError("SSE_PAGINATION_INCOMPLETE")
    elif name == "szse-list.json":
        d = json.loads(data)
        if not isinstance(d, list) or not d or "metadata" not in d[0]:
            raise ValueError("SZSE_JSON_SCHEMA_CHANGED")
        meta = d[0]["metadata"]
        if str(meta.get("subname")) != session:
            raise ValueError("SZSE_JSON_SESSION_UNVERIFIED")
        if int(meta.get("recordcount", 0)) <= 0:
            raise ValueError("SZSE_JSON_EMPTY")
    else:
        ...
        if name == "szse-list.xlsx":
            if not rows or tuple(rows[0][:3]) != ("证券代码", "中文简称", "英文简称"):
                raise ValueError("SZSE_XLSX_HEADER_CHANGED")
        else:
            expected = "Updated as at " + datetime.fromisoformat(session).strftime("%d/%m/%Y")
            if len(rows) < 4 or rows[1][0] != expected:
                raise ValueError("HKEX_SESSION_UNVERIFIED")
            if tuple(rows[2][:5]) != ("Stock Code", "Name of Securities", "Category", "Sub-Category", "Board Lot"):
                raise ValueError("HKEX_HEADER_CHANGED")
    return True
...
def fetch_one(name, spec, out, session, attempts):
    ...
    for i in range(1, attempts + 1):
        attempt = {"attempt": i, "started_at": datetime.now(timezone.utc).isoformat()}
        try:
            with requests.Session() as sess:
                response = sess.get(spec["url"], headers=headers, timeout=(10, 30), allow_redirects=True)
    ...
    row.update(
        status="failed",
        error=row["attempts"][-1]["error"],
        started_at=row["attempts"][0]["started_at"],
        completed_at=row["attempts"][-1]["completed_at"],
    )
    return row
...
def main():
    ...
    rows = [fetch_one(name, spec, args.out, args.session, args.attempts) for name, spec in SOURCES.items()]
    complete = all(x["status"] == "received" for x in rows)
    receipt = {
        "schema_version": 1,
        "run_id": "TRI-DSA-O-SOURCE-RECOVERY-20260918-058U",
        "session": args.session,
        "complete_official_bundle": complete,
        "sources": rows,
    ...
    encoded = json.dumps(receipt, ensure_ascii=False, indent=2) + chr(10)
    (args.out / "source-recovery.json").write_text(encoded)
    (args.out / "source-probe.json").write_text(encoded)
