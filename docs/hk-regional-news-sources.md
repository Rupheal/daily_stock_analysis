...
2026-09-12 14:57（香港时间）云端验收通过，运行：[34679390464](https://github.com/Rupheal/daily_stock_analysis/actions/runs/34679390464)，代码提交 `54e899dfbac200b81ca31500ca9726dee373e46e`。158项回归测试全部通过，覆盖原生港股新闻、来源过滤、发布时间、数据库消费、SearchService及行情校验；7条第三方依赖弃用警告没有导致失败。此次不是完整后端CI或模型输出验收。

| 真实运行通道 | 近期小米关联新闻 | 状态 |
| --- | ---: | --- |
| 经济通公司新闻 | 2 | 明确文章日期及原文链接；入库并进入DSA上下文 |
| 富途公开署名转载 | 5 | AASTOCKS 3条、道琼斯1条、MT Newswires 1条；另5条因原媒体不符合规则被排除 |
| 香港电台财经RSS | 1 | 小米ADR市场报道；入库并进入上下文 |
| 经济通编辑精选/专题RSS | 0 / 0 | 抓取与解析正常，当前窗口没有小米匹配 |
| FT 香港/亚太RSS | 0 / 0 | 抓取与解析正常，当前窗口没有小米匹配 |

合计8条、5个出版机构；窗口为抓取时点前3天，实际保留文章时间为9月11日至12日。全部8条在真实数据库中保存，并由真实DSA消费者读入；插入的旧大陆来源测试哨兵没有进入上下文。港股管线没有触发通用NewsNow刷新。故障隔离、缺日期、过期、未来日期、错误公司和不明来源由离线反例测试覆盖，本次真实7个通道均未报抓取错误。运行产物 `hk-news-integration` 包含news.json、dsa-news-context.txt、daily.json，Actions保留7天；下述统计及运行链接长期保留在此文档。

8条是相关文章数，不代表8个独立事件或8条已证实事实：经济通与AASTOCKS各有同一里昂研报的报道；宁德时代评级稿因摘要提及小米供应商策略而纳入产业链背景；ADR报道不能代替01810港元行情；道琼斯该条为其明确标注的自动生成行情报道。当前去重限于URL和规范化标题，跨语言事件合并、全文事实复核及公告自动下载仍有缺口，不能据此生成已核实的交易结论。

同次回归顺带确认腾讯日线138条，最新完整交易日2026-09-11，收盘26.36港元；此次仅复验可用性与日期，没有新增第二行情源交叉核对。没有调用模型、生成交易方案或启动定时任务。

首次运行 [34679155265](https://github.com/Rupheal/daily_stock_analysis/actions/runs/34679155265) 为148通过/9失败：旧测试使用当地时间及未来时间作为UTC库字段，且一项允许无链接的官方署名。已将测试时间改为UTC历史时间、关闭离线测试自动联网，并按本轮原文链接规则调整反例断言；保留生产日期和来源校验。另修复港股仍启动通用大陆源的实际调用问题，再运行通过以上最终验收。
