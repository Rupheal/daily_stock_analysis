...
- [测试] Run048当前代码136项云端检查通过，私密原件与U独立修正版完成保存/读回/哈希校验；本轮3次模型HTTP估算0.151400元，实际扣费未核实。

- [修复] Run040 拒绝原生 success=false/错误兜底及 SSE 空正式回答、length 截断；免费回放真实失败原件，104项云端测试通过。既有模板、上游代码、原始结果、运行授权不改写。
- [修复] Run037 使用私密分包保留完整字节，约4.9MB真实旧原件保存/读回/哈希/复原通过；模型调用前先验证私有证据保存。原有单对象4MiB上限与凭据检查保持。
- [验收] Run036 原版数据 CLI 实际接收598/660当前21日输入，62失败保留；Run038逐项检查U45并复算44只公开行情事实，正式U信号仍未通过。
- [记录] Run034/039新增2次必要模型HTTP，保守用量估算合计CNY0.144988，实际扣费未核实；两个失败Case封存，未产生模拟或真实成交。

- [修复] Run034-CONT1 将新逐股预检绑定到实际原生输入的全部日线，并在模型前核对数据库每条价格和成交量；小米旧版60日预检保持不变，未用于本次输入的旧档案差异保留隔离记录。
- [改进] Run035 按已冻结 Run006 的21交易日恢复规则核对新时点未复权数据；数据恢复与预测有效性分开验收。

- [改进] Run034 将已验价格/新闻及私有原生证据链扩展到独立股票代码，采用新 Claim、余额检查和每股一次 CNY0.10 上限；正式输出仍需逐项验收，局部样本不发布全池排名。

- [修复] Run033 保存原生全池日线白名单缓存并进行逐股独立历史核验；保留同源恢复、复权差异及成交量失败，不把数据覆盖当作正式出票。

- [修复] O验收观察器支持独立逐股代码校验；区分当前股池生效日与上一完整行情日，新增Run032全池数据及U逐股证据交接。
...
- [改进] 增加获准8元批次的原生DSA用量观察与发送前预算保护，保留原始提示词和报告。

- [新功能] 新增不产生交易结论的港股历史机制回放和不可覆盖的前向观察记录；保留七发动机缺口、原生双轨覆盖、全池预算边界与历史时点限制。

- [测试] 新增隔离原生数据入口验收：U保留45股分母，O单股探针不冒充全池；保留逐股失败、超时、数据库和原生提交证据，不调用模型。

- [修复] 事件去重仅合并已复核的链接与日期，保留后续监管进展和券商修订为新候选。
- [改进] 新增受约束单股研究消费者，模型选择证据引用与判断类别，事实数值仍由程序生成；不代表自由报告或策略有效性已通过。

- [改进] 小米事实简报可选接入逐字段复核的公司业绩及回购原件，保留期间、单位、页码和文件哈希；旧财务聚合数据仍隔离。
- [文档] 登记持续研究goal的新单股调用授权和固定验收清单，保留历史费用及事实V1验收。

- [新功能] 新增小米固定事实简报入口，复用两源日线校验，确定性展示均线、K线与新闻风险索引，无需模型调用。

- [修复] 新增真实单次输出暴露“而非”“尚不构成”被误当肯定回踩；修正否定范围并防止转折后的肯定断言被掩盖。原始输出不变进行免费回放，追加模型额度用完后关闭收费入口。

- [测试] 用户追加授权一次小米单股验收，保留既有两次请求计数，累计上限三次、单次预留0.40元和累计1.20元拦截；免费回归与新鲜输入先行，原始来源字段完整保留，完成后关闭本次入口。

- [修复] 第二次最终验收发现否定句误报和残留基本面来源链：保留否定语义，隔离顶层来源元数据并在模型前检查，来源展示改由确定性字段生成；原始不合格报告继续拦截。本轮两次模型额度已用完，关闭收费入口。

- [修复] 第一份最终验收报告人工复核发现均线排列与日变化混淆、缩量回踩及无风险断言；新增真实回放，按核验值生成均线描述与逐事件新闻引用，隔离仅正文提及公司的行业资讯，合并里昂研报跨语言转载，观察模式不启用执行标志。

- [修复] 小米未解决风险跨新闻窗口保留；按事件与来源区分证据，执行字段只允许结构化单一定义并在直接分析及两条管道共享拦截。免费验收增加官网公告目录、原子检查点与文件哈希，模型请求增加发送前额度预留及重试限制。

- [文档] 小米单股验收增加按运行号与来源哈希接续的状态清单，明确人工未通过及财务隔离范围，并迁入第三阶段已验证的证据与计数方法，制定最终测试要求。
